...
// AESECBPKCS5Decrypt AES解密
func AESECBPKCS5Decrypt(crypted, key []byte) []byte {
	block, err := aes.NewCipher(key)
	if err != nil {
		return []byte("")
	}
...
// AESECBPKCS5Encrypt AES加密
func AESECBPKCS5Encrypt(src, key string) []byte {
...
// Aes cbc 加密, pkcs5 填充
func ByqpAesCBCPk5Encrypt(src, key, iv []byte) ([]byte, error) {
	result, err := Aes128Encrypt(src, key, iv)
	if err != nil {
		return nil, err
	}
	return []byte(base64.RawStdEncoding.EncodeToString(result)), nil
}
...
// AesECBEncrypt aes ECB 模式加密, 标准库没有aes的 ecb 加密模式, 因为 aes 的 ecb 模式加密不安全
